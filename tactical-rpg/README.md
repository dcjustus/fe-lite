...
| Inspect an enemy | Left-click a red unit (view stats only — no control) |
| Move | Click inside the blue circle |
| Switch selection | Click another blue unit while selecting |
| Undo move | Choose **Back** in the action menu, or press **Escape** |
...
All stats have ±15% random variance on spawn, so no two units are identical. MOV determines how far a unit can travel per turn (MOV 5 = large range, MOV 3 = short range).
...
- **Archer** — fast, hits from long range, but has a **dead zone** and can't attack enemies that are too close.
...
- **Bow** — ranged only; cannot target enemies within ~80 units of distance. The dead zone is shown visually as a dark inner ring.
...
**Item drops** — when a unit is defeated, each item they carried has a 30% chance to go directly into the killer's inventory. The rest scatter to the ground as golden diamonds; walk any unit over them to automatically pick them up.
...
| Small circle + bow arc | Archer (Bow) |
| Diamond | Mage (Magic) |
...
- Clicking a **red enemy** on your turn shows their stats and attack range in view-only mode (gold ring around the unit, stats in the right panel). You cannot control them.
...
- Archers need room; don't back them into a corner where enemies close to melee.
